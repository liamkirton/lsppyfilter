...
def http_request_filter(pid, tid, sid, request):
	...
	#
	# Replace all "GET * HTTP/1.1" requests with "GET /robots.txt HTTP/1.1"
	#
	request = re.compile('(GET\s*)(.*?)(\s*HTTP/1.1)', re.DOTALL | re.IGNORECASE | re.MULTILINE).sub('\g<1>/robots.txt\g<3>', request)
	return request
...
if __name__ == '__main__':
	...
	print '\"Filters\\Http_Robots-Example.py\" Loaded.'
