...
def http_request_filter(pid, tid, sid, request):
	print '[%x, %x, %x] http_request_filter()' % (pid, tid, sid)
	return request

# ========================================================================================================================
	
def http_response_filter(pid, tid, sid, header, content):
	print '[%x, %x, %x] http_response_filter()' % (pid, tid, sid)
	
	#
	# Replace all img src='*' with a cat picture
	#
	content = re.compile('(img.*?src=[\'\"])(.*?)([\'\"].*?>)', re.DOTALL | re.IGNORECASE | re.MULTILINE).sub('\g<1>http://newsimg.bbc.co.uk/media/images/44021000/jpg/_44021735_oscar_ap66i.jpg\g<3>', content)
...
if __name__ == '__main__':
	...
	print '\"Filters\\Http_Cats-Example.py\" Loaded.'
